...
function getCategory_List(e) {
	...
	if(obj.category_lv == 1 || obj.category_lv == 2) {
		...
		//클릭한 대상에 data-set값이 obj에 있습니다
		$.ajax({
			type: "post",
			url: "../getCategoryChild",
			data: JSON.stringify({"group_id" : obj.group_id,
								  "category_lv" : obj.category_lv,
								  "category_detail_lv": obj.category_detail_lv }),
			contentType: "application/json",
			success: function(data) {
				 category_create(data); //태그생성구문호출
	...
	}//if문 밖에서 카테고리를 value에 세팅해주는 함수를 호출 
	$(e.target).category_set(); //매개값이 e.target
